...
path = get_cloud_storage_path(zone='standardizedzone',kind='referencedata',table_name='taxitripmode')
df_trip_mode = read_delta_table(spark,path,'*')
...
(
    ...
    .merge(
        source=df_trip_mode.alias('src'),
        condition = 'src.TripModeID = trg.TripModeID'
    )
    .whenMatchedUpdateAll(
        condition = 'src.TripMode <> trg.TripMode'
    )
