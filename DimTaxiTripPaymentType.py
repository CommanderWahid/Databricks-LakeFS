...
path = get_cloud_storage_path(zone='standardizedzone',kind='referencedata',table_name='taxipaymenttype')
df_trip_payment_type = read_delta_table(spark,path,'*')
...
(
    ...
    .merge(
        source=df_trip_payment_type.alias('src'),
        condition = 'src.PaymentTypeID = trg.PaymentTypeID'
    )
    .whenMatchedUpdateAll(
        condition = 'src.PaymentDesc <> trg.PaymentDesc'
    )
