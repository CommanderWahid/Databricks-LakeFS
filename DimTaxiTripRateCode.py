...
path = get_cloud_storage_path(zone='standardizedzone',kind='referencedata',table_name='taxiratecode')
df_trip_rate_code = read_delta_table(spark,path,'*')
...
(
    ...
    .merge(
        source=df_trip_rate_code.alias('src'),
        condition = 'src.RateCodeID = trg.RateCodeID'
    )
    .whenMatchedUpdateAll(
        condition = 'src.RateCodeDesc <> trg.RateCodeDesc'
    )
